...
function findIntersectingLayerWithPoint(layerIds, layers, point) {
  for (let i = layerIds.length - 1; i >= 0; i--) {
    const layerId = layerIds[i];
    const layer = layers.get(layerId);
    if (layer && isHittingLayer(layer, point)) {
...
function isHittingLayer(layer, point) {
  switch (layer.type) {
    case LayerType.Ellipse:
      return isHittingEllipse(layer, point);
    case LayerType.Path:
...
function isHittingRectangle(layer, point) {
  return (
    point.x > layer.x &&
    point.x < layer.x + layer.width &&
    point.y > layer.y &&
...
function isHittingEllipse(layer, point) {
  const rx = layer.width / 2;
  const ry = layer.height / 2;
  const cx = layer.x + rx;
  const cy = layer.y + ry;
